...
import { getUserData, getUserRole, showLoader, hideLoader } from '../../../service/Common.js';
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        currentTimeStatusFun()
    }, []);
    useEffect(() => {
        getCompanyDashBoardData(reqData)
    }, [gender, bmi, age])
    //-------------- callback functions -------------//
    const currentTimeStatusFun = () => {
        var today = new Date()
    ...
    function getCompanyDashBoardData(reqData) {
        ...
        getCompanyDashBoard(reqData).then(result => {
            ...
            if (result.data.response.totalHraScore) {
                setTotalHraScore(result.data.response.totalHraScore.toFixed(2))
            } else {
                setTotalHraScore(result.data.response.totalHraScore)
            }

            if (result.data.response.catScoreAvg['My Body']) {
                setBody(result.data.response.catScoreAvg['My Body'].toFixed(2))
            } else {
                setTotalHraScore(result.data.response.catScoreAvg['My Body'])
            }

            if (result.data.response.catScoreAvg['My Lifestyle']) {
                setMind(result.data.response.catScoreAvg['My Lifestyle'].toFixed(2))
            } else {
                setTotalHraScore(result.data.response.catScoreAvg['My Lifestyle'])
            }

            if (result.data.response.catScoreAvg['My Mind']) {
                setLifestyle(result.data.response.catScoreAvg['My Mind'].toFixed(2))
            } else {
                setTotalHraScore(result.data.response.catScoreAvg['My Mind'])
            }
            //------------ for catScoreAvg -----------//
                                ...
                                <div className="row mb-2 mb-lg-4">
                                    <div className="col-lg-12">
                                        <div className="bg-white shadow_d p-4 rounded-3 dashboard_bg_img ">
                                            <h2>{currentTime} {getUserData().response.full_name}</h2>
                                                            ...
                                                            <div key={index} className='col text-center'>
                                                                ...
                                                                <label htmlFor={"gender" + item.id}>
                                                                    ...
                                                                    <div className="tick_container" style={{ opacity: item.id == gender? '1': '0'}}>
                                                                        <div className="tick" style={{background: item.id == gender? 'rgba(66, 143, 236, 0.5)': '0'}}>&nbsp;</div>
                                                                    </div>
                                                                </label>
